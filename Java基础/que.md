1. 谈谈对java多态的理解？
   ...
   * 多态的定义：即同一消息可以根据发送对象的不同而采用多种不同的行为方式。（发送消息就是函数调用），同一事件发生在不同的对象上会产生不同的结果。比如：按下 F1 键这个动作，如果当前在 Flash 界面下弹出的就是 AS 3 的帮助文档；如果当前在 Word 下弹出的就是 Word 帮助；在 Windows 下弹出的就是 Windows 帮助和支持。
...
3. 简单介绍一下java中的泛型，泛型擦除以及相关的概念，解析与分派？
    ...
    * 泛型的好处：编译的时候检查类型安全，并且所有的转换都是自动和隐式的；可以将；类型当做参数传递给一个类或者方法，不需要对结果进行强制类型转换
    * 泛型的参数类型可以使用extends语句，例如。习惯上称为“有界类型”。
    * 泛型的参数类型还可以是通配符类型。例如Class<?> classType = Class.forName("java.lang.String");
    * 上界通配符：extends，适合用于频繁往外读取内容的
    * 下界通配符：super，适合用于经常往里插入的
4. 泛型擦除以及相关的概念
    * 泛型信息只存在代码编译阶段，在进入JVM之前，与泛型关的信息都会被擦除掉。即泛型信息在源代码中有，在生成的字节码文件里面就没有了
    ...
    * 使用泛型的时候加上的类型参数，会在编译器在编译的时候擦除掉。生成的Java字节码中是不包含泛型中的类型信息的。这个过程就称为类型擦除。
    ...
    ```
    输出结果是true
    * 泛型被擦除后，还可以通过反射获取到泛型信息。类，字段和方法的形参泛型的信息被保存在签名文件中，
5. 说说你对Java注解的理解？
...
6. Java的匿名内部类有哪些限制？
    ...
    * 匿名内部类只能访问final修饰的外部变量
        * 原因：匿名内部类最终会编译成一个单独的类，而被该类使用的变量会以构造函数参数的形式传递给该类。
        * 如果不是final的，那么这个变量在内部会被修改，会造成与外部不一致的问题
        * 因此只能访问final修饰的外部变量
...
15. final，finally，finalize的区别？
    * final 可以用来修饰类、方法、变量，方法参数，分别有不同的意义，final 修饰的 class 代表不可以继承扩展，final 的变量是不可以修改的，final 的方法也是不可以重写的（override），final修饰参数表示不能再改变方法参数的值
      * 被final修饰的变量，不可变的是变量的引用，而不是变量的内容。
      * 修饰基本数据类型时，值不能改变
      * 修饰引用类型时，限定了引用变量的引用不可改变，即不能将该对象再次引用另一个对象，但是引用的对象的值是可以改变的，
      * 修饰变量时，在声明时或者构造方法中对它赋值；
...
22. 抽象类与接口的区别：
    ...
    * 抽象类是对类的抽象
    * 接口中的变量都是final类型，抽象类可以包括非final的变量
    * 接口：规范、扩展、回调
    * 抽象类：为其子类提供一个公共的类型 封装子类中得重复内容 定义抽象方法，子类虽然有不同的实现 但是定义是一致的
23. 分别讲讲 final，static，synchronized 关键字可以修饰什么，以及修饰后的作用？
    * static：不依赖于任何对象就可以进行访问。静态变量在内存中只有一个副本，只有类初次加载时会被初始化。非静态变量在创建对象的时候被初始化，存在多个副本。static方法不能被覆盖，因为方法覆盖的实现是运行时动态绑定的，而static是编译时静态绑定的，并且在类加载时就加载，因此不能覆盖。
...
```
   ...
   * 用反射可以修改String
37. 当一个对象被当做参数传递到一个方法时，此方法可改变对象的属性并可返回变化后的结果，那么这里是值传递还是引用传递？
    * java只有值传递参数。当一个对象被作为参数传递到方法中时，改变的是对象的内容而不是对象的引用。
38. java闭包：指函数和函数内部能访问到的变量（也叫环境）的总和，就是一个比好
39. IO、NIO、OKIO
...
40. Java中的继承是单继承还是多继承
    * Java中既有单继承，又有多继承。对于java类来说只能有一个父类，对于接口来说可以同时继承多个接口
41. 字节数
    * byte 位数 8，字节数 1。 short 位数 16， 字节数 2。 int 32，4
