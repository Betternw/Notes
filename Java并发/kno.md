...
### synchronized锁升级
1. 最开始是无锁状态
2. 第一次访问时，JVM设置为偏向锁。将线程id设置为当前线程的id，再次进入时判断id是否一致
3. 不一致时，升级为轻量锁，进入自旋
4. 自旋一定次数后仍未获取资源，升级为重量锁。。

### Volatile
#### 理解
1. 主要作用是能够保证有序性和可见性，不能保证原子性
2. 有序性通过插入内存屏障实现
3. 可见性通过，每次把修改后的值立即更新到主存，其他线程的变量副本失效，需要再到主存读取。

#### 内存屏障
1. 在适当位置插入内存屏障阻止重排序

#### 不保证原子性
1. Java对基本类型的赋值和读取是原子操作，如 i=1。但 i++ 不是原子操作，实际上其被拆成了3个指令，在将修改写回主内存时会产生覆盖，所以不能保证原子性。
2. 解决方法：加锁，加synchronized，原子类。

#### synchronized和volatile的区别
1. volatile是轻量级的锁，性能比synchronized好
2. volatile只能修饰变量，synchronized能修饰方法和代码块
3. volatile保证数据可见性和有序性，不保证原子性；synchronized都保证。

### Lock接口
Lock接口是synchronized的升级版，支持非公平和公平锁，提供了轮询、定时、中断、多条件的锁操作，更加灵活。
### ReentrantLock可重入锁（可中断、默认非公平）
1. 是java.util.concurrent（J.U.C）包中的锁。
2. 是Lock接口的一个实现类
3. 可重入锁就是自己可以再次获取自己的内部锁。同一线程 外层函数获得锁之后 ，内层递归函数仍然有获取该锁的代码。ReentrantLock 和synchronized 都是 可重入锁。同一线程每次获取锁，锁的计数器++，计数器为0时再释放锁。
### 比较——实现、修饰、等待中断、公平、通知
1. 锁的实现：synchronized 是 JVM 实现的，而 ReentrantLock 是 JDK 实现的。
2. 二者都是可重入锁。
3. synchronized修饰类和方法，ReentrantLock只适用代码块。
4.  等待可中断：当持有锁的线程长期不释放锁的时候，正在等待的线程可以选择放弃等待，改为处理其他事情。ReentrantLock 可中断，而 synchronized 不行。
5.  公平锁：公平锁是指多个线程在等待同一个锁时，必须按照申请锁的时间顺序来依次获得锁。synchronized 中的锁是非公平的，ReentrantLock 默认情况下也是非公平的，但是也可以是公平的。
6.  支持选择性通知。synchronized相当于整个Lock只有一个Condition，所有线程都注册在一个上面，notifyAll()通知所有等待状态线程，效率不高。ReentrantLock的线程对象能注册在指定的Condition中，signalAll()只会唤醒该Condition实例中的等待线程。
### 使用选择
...
构建锁和同步器的框架。
#### 底层原理
1. 若线程请求的资源空闲，将线程设置为工作线程，资源被分配后将共享资源设置为锁定状态
2. 若线程请求的资源被占用，就将暂时获取不到锁的进程封装成一个节点加入一个队列中
3. AQS的作用就是将获取不到锁的线程封装成节点加入到队列中，虚拟双向队列
4. 内部使用FIFO的排队策略，使用CAS进行值的修改，使用volatile int变量的state标识同步状态，
