...
3. 信号量 Semphares：允许同一时刻多个线程访问同一资源，但会控制同时访问的最大线程数量。
4. 管程：独立出控制的代码，使客户端调用代码更容易。在一个时刻只能有一个进程使用管程。
5. 互斥量 Mutex：只有持有互斥对象的线程才能访问公共资源，保证公共资源不会被同时访问。比如 synchronized锁，Lock类。
6. 
#### 进程通信
...
## 4. 内存管理
#### 管理机制
* 连续分配管理：为程序分配一个连续的内存空间。
    * 块式管理：将内存分为若干个块，每个块只能分配一个进程，未被利用的空间被称为碎片。
* 非连续分配管理：为程序分配不连续的内存空间。但每个页/段内的地址是连续的。
    * 页式管理：将内存分为若干大小相同的页，通过页表对应逻辑和物理地址。内存划分得更小，利用率更高，少量内部碎片。
    * 段式管理：将内存分为若干段，内存划分得更小，且有实际意义，每个段定义一组逻辑信息，通过段表管理。段过长也要分配连续空间。
    * 段页式管理：结合两者优点。先把内存划分成段，每个段划分为页。
* 快表和多级页表
    * 一是虚拟地址到物理地址的转换；二是虚拟地址空间过大。
    * 快表
      * 根据虚拟地址中的页号查快表
      * 该页在快表中，直接读取。
      * 该页不在快表中，到内存中读取。并把该页表添加到块表中
      * 快表需要淘汰机制进行淘汰内部的页表。
    * 多级页表
      * 避免所有的页表都放在内存中。将二级及之后的页表放在磁盘中，用到时再调用，
#### 虚拟内存
1. 让物理内存扩充成更大的逻辑内存
